        ...
        reader.onload = (e) => {
            try {
                ...
                if (file.name.endsWith('.json')) {
                    parsedDb = JSON.parse(content);
                } else if (file.name.endsWith('.html')) {
                    const parser = new DOMParser();
                    const doc = parser.parseFromString(content, 'text/html');
                    const scriptTag = doc.getElementById('design-board-data');
                    if (scriptTag && scriptTag.textContent) {
                        parsedDb = JSON.parse(scriptTag.textContent);
                    } else {
                        throw new Error("Arquivo HTML inválido ou sem dados do projeto.");
                    }
                }

                if (parsedDb) {
                    setDb(parsedDb);
                    setNotification("Projeto Carregado com Sucesso!");
                    setTimeout(() => setNotification(null), 3000);
                    setShowAdminModal(false);
                    addLog("SISTEMA", "BACKUP RESTAURADO");
                }
            } catch (err) {
                ...
                <div className="grid grid-cols-1 lg:grid-cols-12 gap-6 no-print">
                     <div className="lg:col-span-4 flex flex-col gap-6">
                        <div className="grid grid-cols-2 gap-4">
                            <div className={`ds-card-accent cursor-pointer p-4 flex flex-col justify-center items-center text-center h-48 transition-all relative group overflow-hidden ${!hasLod ? 'animate-pulse opacity-80' : 'hover:-translate-y-1'}`} onClick={() => setShowLodModal(true)}>
                                <span className="text-[10px] font-bold text-white/80 uppercase tracking-widest mb-1 flex items-center gap-1 border border-white/20 px-2 py-0.5 rounded-full bg-white/10">1. Fase <span className="material-symbols-outlined text-xs">chevron_right</span></span>
                            ...
                            <div className={`ds-card-accent p-4 flex flex-col items-center justify-center text-center h-48 transition-all relative ${!hasLod ? 'opacity-30 grayscale cursor-not-allowed' : 'cursor-pointer hover:-translate-y-1'}`} onClick={() => hasLod && setShowCompanyModal(true)}><span className="text-[10px] font-bold text-white/80 uppercase tracking-widest mb-2 flex items-center gap-1 border border-white/20 px-2 py-0.5 rounded-full bg-white/10">2. Cliente <span className="material-symbols-outlined text-xs">chevron_right</span></span><h2 className="font-square font-black text-white uppercase text-lg truncate w-full px-2 mt-1">{activeCompany?.name || 'Selecione'}</h2>{hasCompany && <span className="material-symbols-outlined absolute right-3 bottom-3 text-white/30 text-2xl">check_circle</span>}</div>
                            
                            <div className={`ds-card-accent p-4 flex flex-col items-center justify-center h-48 transition-all relative group overflow-hidden ${!hasCompany ? 'opacity-30 grayscale cursor-not-allowed' : 'cursor-pointer hover:-translate-y-1'}`} onClick={() => hasCompany && setShowProjectModal(true)}>
